...
# Advantage Masternode Setup Guide
***
## Required
1) **USDX collateral value at current block** ([consult the collateral table](../../README.md#rewards-breakdown))
...
Remember to do `advantage-cli getblockcount` to check if VPS catching blocks till it synced with chain, if not follow this procedure:

* Go to your wallet-qt and check peers list (tools - peers list) and select one ip from the list. With that ip do the follow command at VPS `advantage-cli addnode "ip" onetry`

      Example:
		  advantage-cli addnode 45.32.144.158 onetry
    
* Check now if VPS already downloading blocks with the command `advantage-cli getblockcount`, and if yes give it time now to catch last block number 
...
```
advantage-cli getmasternodestatus
```

You need to get **"status" : 4** 

## Congratulations your Advantage node it's running
